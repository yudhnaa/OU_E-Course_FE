	...
	useEffect(() => {
		const fetchQuizData = async () => {
			try {
				...
				const exerciseRes = await Apis.get(
					endpoints["exerciseDetail"](courseId, exerciseId)
				);
				...
				// Fetch questions
				const questionsRes = await Apis.get(
					endpoints["exerciseQuestions"](courseId, exerciseId)
				)
					.then(
						(res) =>
							console.log("Questions fetched successfully:", res.data) || res
					)
					.catch((err) => {
		...
		return questions.map((question) => {
			...
			return {
				...
				score: selectedAnswer?.isCorrect
					? exerciseDetail.maxScore / questions.length
					: 0,
			};
		});
	};
								...
								<Card.Body>
									...
									<ul className="list-unstyled">
										...
										<li>
											...
											{exerciseDetail.durationMinutes} mins
										</li>
										<li>
											<strong>Max Score:</strong> {exerciseDetail.maxScore}
										</li>
									</ul>
								</Card.Body>
							</Card>
